...
test('optional memory, provenance, and canary gates stay fail-closed', () => {
...
test('delegation freshness and approval replay gates are enforced by authorization', () => {
  const now = new Date('2026-08-15T12:00:00Z');
  const delegation = authorize({ ...base, delegationContext: { delegatorTrusted: true, receiverTrusted: true, delegated: true, audienceMatches: true, delegationExpiresAt: '2026-08-15T11:59:00Z' }, now });
  const approval = authorize({ ...base, approvalFreshnessContext: { approvalExpired: true }, now });
  assert.equal(delegation.code, 'delegation-expired');
  assert.equal(approval.code, 'approval-replay-blocked');
});
